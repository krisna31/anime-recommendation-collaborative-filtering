...
```
Bisa dilihat dari hasil rekomendasi hasil rekomendasi memiliki genre yang serupa misalnya *adventure* untuk anime *Ookami to Koushinryou II*, direkomendasikan anime dengan genre yang serupa seperti *Cowboy Bebop, Toki wo Kakeru Shoujo*.
...
Dapat dilihat dari gambar 2 nilai error RMSE turun hingga sesuai dengan target awal kita dimana nilai error dibawah 16% sehingga pada *epoch* ke 6 pelatihan model sudah bisa terhenti dengan *callback* yang terpanggil, yang berarti model memiliki tingkat kesalahan yang relatif rendah atau sebesar 16% dalam memprediksi anime berdasarkan preferensi pengguna atau sebaliknya menunjukkan bahwa model dapat memberikan rekomendasi anime dengan tingkat akurasi yang cukup tinggi, atau mendekati preferensi pengguna sebenarnya dengan tingkat akurasi 84% tepat. Sehingga dari hasil ini dapat disimpulkan bahwasanya model yang kita buat sudah memenuhi tujuan pembuatan model dan dapat diimplementasikan ke aplikasi *balibali*.
